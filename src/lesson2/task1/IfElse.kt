...
fun timeForHalfWay(
    ...
    val halfDistance = ((v1 * t1) + (v2 * t2) + (v3 * t3)) / 2.00
    return when {
        halfDistance < (v1 * t1) -> halfDistance / v1
        halfDistance > ((v1 * t1) + (v2 * t2)) -> t1 + t2 + (((v3 * t3) - halfDistance) / v3)
        else -> t1 + ((halfDistance - (v1 * t1)) / v2)
    }
}
...
fun rookOrBishopThreatens(
    ...
    val differenceX = abs(bishopX - kingX)
    val differenceY = abs(bishopY - kingY)
    return when {
        (differenceX == differenceY) && ((kingX == rookX) || (kingY == rookY)) -> 3
